...
function createMultiLine() {
	...
	c = 0;
	max = 0;
	//寻找最大值(现在由于td里面的结构改为了一个span 一个input所以现在还要改)
	for (i = 1 ;i < trlen;i++) {
		if (trArr[i].childNodes.length == 14) {
			for (j = 2;j < 14;j++) {
				if (max < Number(trArr[i].childNodes[j].querySelector('span').innerText)) {
					max = Number(trArr[i].childNodes[j].querySelector('span').innerText);
				}
			}
		}else {
			for (j = 1; j < 13;j++) {
	...
	for (i = 1 ;i < trlen;i++) {
		if (trArr[i].childNodes.length == 14) {
			for (j = 0;j < sourDatalen;j++) {
				if (trArr[i].childNodes[0].querySelector('span').innerText == sourceData[j].product && trArr[i].childNodes[1].querySelector('span').innerText == sourceData[j].region || trArr[i].childNodes[1].querySelector('span').innerText == sourceData[j].product && trArr[i].childNodes[0].querySelector('span').innerText == sourceData[j].region) {
			...
			for (j = 0;j < sourDatalen;j++) {
				if (trArr[i].childNodes[0].querySelector('span').innerText == sourceData[j].product && temp == sourceData[j].region || temp == sourceData[j].product && trArr[i].childNodes[0].querySelector('span').innerText == sourceData[j].region) {
					cMultiLine('line',sourceData[j],document.defaultView.getComputedStyle(document.getElementById('line'),null).width,document.defaultView.getComputedStyle(document.getElementById('line'),null).height,max,color[c],i);
					c++;
					break;
				}
